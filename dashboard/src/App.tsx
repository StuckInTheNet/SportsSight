...
import { Activity, Eye, Key, Loader2, Play, Radio, Users, Wifi } from "lucide-react";
...
export default function App() {
  ...
  const [allRecords, setAllRecords] = useState<FatigueRecord[]>([]);
  ...
  const handleAuth = useCallback(async (key: string) => {
    ...
    try {
      ...
      setAuthError(null);
      const [gamesList, playersList] = await Promise.all([api.listGames(), api.listPlayers()]);
      setGames(gamesList);
  ...
  useEffect(() => { if (apiKey) handleAuth(apiKey); }, []); // eslint-disable-line
  ...
  useEffect(() => {
    ...
    api.getGameFatigue(selectedGameId, { limit: "10000" })
      .then(setAllRecords)
      .catch((e) => console.error("Load failed:", e))
      .finally(() => setLoading(false));
  }, [selectedGameId, authenticated]);

  // --- Derive display data based on view mode + video time ---

  // Group records by player
  const byPlayer = useMemo(() => {
    const m = new Map<string, FatigueRecord[]>();
  ...
  // In "live" mode, only show data up to current video time
  // In "full" mode, show everything
  const cutoffMs = viewMode === "live" ? videoTimeMs : Infinity;

  // Scores at current time
  const scoresAtTime = useMemo(() => {
    const result = new Map<string, any>();
    for (const [pid, records] of byPlayer) {
      let best: FatigueRecord | null = null;
      for (const r of records) {
        if (r.timestamp_ms <= cutoffMs) best = r;
        else break;
      }
      if (best) {
        result.set(pid, {
    ...
    return result;
  }, [byPlayer, cutoffMs, liveScores]);

  // Timeline data (filtered by cutoff)
  const timelineData = useMemo(() => {
    const byTime = new Map<number, Map<string, FatigueRecord>>();
    for (const r of allRecords) {
      if (r.timestamp_ms > cutoffMs) continue;
    ...
    for (let i = 0; i < timestamps.length; i += step) {
      ...
      for (const [pid, rec] of byTime.get(ts)!) entry[pid] = rec.fatigue_score;
      tl.push(entry);
    }
    return tl;
  }, [allRecords, cutoffMs]);
  ...
  const playerName = useCallback((pid: string): string => {
    ...
    return `P-${pid.slice(0, 4)}`;
  }, [dbPlayers]);

  // Top 10 players by fatigue
  const displayPlayers: Player[] = useMemo(() => {
    return Array.from(scoresAtTime.entries())
      ...
      .map(([pid]) => {
        ...
        if (p) return { ...p };
        const num = parseInt(pid, 10);
        return { id: pid, name: `Player ${num < 100 ? num : pid.slice(0, 6)}`, jersey_number: !isNaN(num) && num < 100 ? num : null, position: null };
      });
          ...
          <div className="flex gap-2">
            <div className="flex-1 relative">
              ...
              <input
                ...
                onKeyDown={(e) => { if (e.key === "Enter") handleAuth((e.target as HTMLInputElement).value.trim()); }}
              />
            </div>
            <button
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100">
          ...
          <div className="flex items-center gap-3">
            ...
            {selectedGameId && (
              <div className="flex bg-gray-800 rounded-lg border border-gray-700 p-0.5">
                ...
                >
                  <Eye className="w-3 h-3" /> Full Game
                </button>
              </div>
            )}
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-5">
        {wsError && (
          <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 text-sm text-red-400 mb-4">{wsError}</div>
        )}
            ...
            <div className="lg:col-span-2 space-y-5">
              <section>
                <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wide mb-3">
                  ...
                  {viewMode === "live" && videoTimeMs === 0 && (
                    <span className="text-gray-500 ml-2 normal-case tracking-normal font-normal">
                      — press play to reveal data
                    </span>
                  )}
                </h2>
