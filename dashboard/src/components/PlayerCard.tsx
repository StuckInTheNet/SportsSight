import type { FatigueScore, Player } from "@/types";
    ...
    <div className={`rounded-xl border p-4 ${levelBg(level)}`}>
      ...
      {fatigue && (
        <div className="grid grid-cols-2 gap-2 mt-3 text-xs text-gray-400">
          ...
          <div>
            <span className="block text-gray-500">Predicted (5m)</span>
            <span className={levelColor(
              fatigue.predicted_score_5min >= 75 ? "high" :
              fatigue.predicted_score_5min >= 55 ? "moderate" : "low"
            )}>
              {fatigue.predicted_score_5min.toFixed(0)}
            </span>
          </div>
        </div>
      )}
